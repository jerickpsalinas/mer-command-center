  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <ChevronDown className={`${compact ? "h-3 w-3" : "h-3.5 w-3.5"} opacity-60`} />
      </button>
      {open && (
        <div className="absolute right-0 mt-1.5 w-56 rounded-lg border border-border bg-popover shadow-lg z-50 overflow-hidden">
          {options.map((opt) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between gap-2 px-3 py-2 text-[12px] text-left transition-colors ${
