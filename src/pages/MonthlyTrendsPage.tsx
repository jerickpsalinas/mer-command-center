...
export default function MonthlyTrendsPage() {
  const [selectedMonth, setSelectedMonth] = useState(monthlyTrends.length - 1);
  const current = monthlyTrends[selectedMonth];
  const previous = selectedMonth > 0 ? monthlyTrends[selectedMonth - 1] : null;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        >
          {monthlyTrends.map((t, i) => (
            <option key={t.month} value={i}>{t.month}</option>
          ))}
        </select>
        {previous && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPICard
          title="Compliant"
          value={current.compliant}
          icon={CheckCircle2}
          variant="success"
          trend={previous ? `${compDiff >= 0 ? "+" : ""}${compDiff} vs last month` : undefined}
          index={0}
        />
        <KPICard
          title="Non-Compliant"
          value={current.nonCompliant}
          icon={XCircle}
          variant="destructive"
          index={1}
        />
        <KPICard
          ...
          icon={TrendingUp}
          trend={previous ? `${pctDiff >= 0 ? "+" : ""}${pctDiff}pp vs last month` : undefined}
          index={2}
        />
        <KPICard
          title="Trend"
          value={pctDiff >= 0 ? "Improving" : "Declining"}
          icon={pctDiff >= 0 ? TrendingUp : TrendingDown}
          variant={pctDiff >= 0 ? "success" : "destructive"}
          index={3}
        />
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {monthlyTrends.map((t, i) => (
              <tr
                key={t.month}
                className={`border-b border-border hover:bg-accent/50 transition-colors ${i === selectedMonth ? "bg-primary/5" : ""}`}
              >
