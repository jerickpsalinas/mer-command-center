...
import { getKPIMetrics, getComplianceBreakdown, getNeedsAttention, getBookkeeperStats, monthlyTrends } from "@/data/mockData";
  ...
  const sections = [
    {
      ...
      count: attention.missingStatements.length,
      icon: FileText,
      priority: "critical" as const,
      items: attention.missingStatements.map(c => ({
        id: c.id,
        label: c.name,
        badge: <StatusBadge status="Non-Compliant" />,
      })),
    },
    {
      title: "Unresolved Transactions",
      count: attention.unresolvedTransactions.length,
      icon: AlertCircle,
      priority: "high" as const,
      items: attention.unresolvedTransactions.slice(0, 6).map(c => ({
        id: c.id,
        label: c.name,
        badge: <span className="font-mono-data text-xs font-semibold text-destructive">{c.uncategorizedTransactions}</span>,
      })),
    },
    {
      title: "Not Reconciled",
      count: attention.notReconciled.length,
      icon: Clock,
      priority: "medium" as const,
      items: attention.notReconciled.slice(0, 5).map(c => ({
        id: c.id,
        label: c.name,
        badge: <span className="font-mono-data text-xs text-muted-foreground">{c.lastReconciledDate}</span>,
      })),
    },
    {
      title: "No Approved Notes",
      count: attention.noApprovedNotes.length,
      icon: StickyNote,
      priority: "medium" as const,
      items: [],
    },
    ...
    >
      <div className="px-6 py-4 border-b border-border bg-destructive/[0.02]">
        <h2 className="text-sm font-semibold text-foreground flex items-center gap-2">
          ...
          Needs Attention
          <span className="ml-auto text-xs font-mono-data text-destructive font-semibold">
            {sections.reduce((sum, s) => sum + s.count, 0)} issues
          </span>
        </h2>
      </div>
          ...
          <div key={section.title} className="p-5">
            <div className="flex items-center gap-2 mb-3">
              <span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider px-2 py-1 rounded-md border ${priorityStyles[section.priority]}`}>
                ...
                {section.priority === "critical" ? "Critical" : section.priority === "high" ? "High" : "Medium"}
              </span>
              <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider ml-auto">
                {section.count}
              </span>
            </div>
...
function BookkeepersSection() {
  const bkStats = getBookkeeperStats();
    ...
    >
      ...
      <div className="p-5 space-y-3">
        {bkStats.map((bk, i) => (
          <div key={bk.name} className="flex items-center gap-3 p-2 rounded-lg hover:bg-accent/50 transition-colors -mx-2">
            <div className="h-9 w-9 rounded-lg bg-primary/8 flex items-center justify-center text-xs font-bold text-primary">
              #{i + 1}
            </div>
      ...
      <div className="border-t border-border px-6 py-4">
        ...
        <div className="space-y-2.5 text-[12px] text-muted-foreground">
          <p>Creative Pathways marked <span className="text-success font-semibold">Compliant</span></p>
          <p>Fathers H.A.R.B.O.R marked <span className="text-success font-semibold">Compliant</span></p>
          <p>MAI Academy flagged — <span className="text-destructive font-semibold">missing statements</span></p>
        </div>
      </div>
    </motion.div>
  );
...
export default function DashboardPage() {
  ...
  const breakdown = getComplianceBreakdown();
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={monthlyTrends}>
              ...
              />
              <Line type="monotone" dataKey="compliant" stroke="hsl(var(--success))" strokeWidth={2.5} dot={{ r: 3.5, strokeWidth: 2 }} name="Compliant" />
              <Line type="monotone" dataKey="nonCompliant" stroke="hsl(var(--destructive))" strokeWidth={2.5} dot={{ r: 3.5, strokeWidth: 2 }} name="Non-Compliant" />
            </LineChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={monthlyTrends}>
              ...
              />
              <Bar dataKey="completionPct" fill="hsl(var(--primary))" radius={[6, 6, 0, 0]} name="Completion %" />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
      </div>
