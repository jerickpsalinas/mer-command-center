import { getKPIMetrics, getComplianceBreakdown, getBookkeeperStats, clients, getNeedsAttention } from "@/data/mockData";
      ...
      content: (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-2xl font-mono-data font-semibold text-success">{kpi.compliant}</p>
              <p className="text-xs text-muted-foreground">Compliant</p>
            </div>
            <div>
              <p className="text-2xl font-mono-data font-semibold text-destructive">{kpi.nonCompliant}</p>
              <p className="text-xs text-muted-foreground">Non-Compliant</p>
            </div>
            <div>
              <p className="text-2xl font-mono-data font-semibold text-foreground">{kpi.avgCompletion}%</p>
      ...
      content: (
        <div className="space-y-3">
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">Missing Statements</span>
            <span className="font-mono-data text-destructive font-medium">{attention.missingStatements.length}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">Not Reconciled</span>
            <span className="font-mono-data text-destructive font-medium">{attention.notReconciled.length}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">Unresolved Txns</span>
            <span className="font-mono-data text-warning font-medium">{attention.unresolvedTransactions.length}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">No Approved Notes</span>
            <span className="font-mono-data text-destructive font-medium">{attention.noApprovedNotes.length}</span>
          </div>
      ...
      content: (
        <div className="space-y-2">
          <div className="grid grid-cols-3 gap-2 text-center pb-3 border-b border-border">
            <div>
              <p className="text-lg font-mono-data font-semibold text-foreground">{clients.filter(c => c.clientType === "School").length}</p>
              <p className="text-xs text-muted-foreground">Schools</p>
            </div>
            <div>
              <p className="text-lg font-mono-data font-semibold text-foreground">{clients.filter(c => c.clientType === "For-Profit").length}</p>
              <p className="text-xs text-muted-foreground">For-Profit</p>
            </div>
            <div>
              <p className="text-lg font-mono-data font-semibold text-foreground">{clients.filter(c => c.clientType === "Non-Profit").length}</p>
